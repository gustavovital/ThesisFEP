# script providing variable selection adaptative lasso:
# 
# ADAPTATIVE LASSO for variable selection
# 
# Gustavo Vital
...
sink(file = "data\\adaptative_lasso_output.txt")
...
for(i in 1:6){
  ...
  ridge <- glmnet(x = train.sparse, 
                  y = y_train,
                  alpha = 0)
  
  cv.ridge <- cv.glmnet(x = train.sparse, 
                        y = y_train,
                        alpha = 0)
  
  best.ridge <- as.numeric(coef(cv.ridge, s = cv.ridge$lambda.min))[-1]
  
  # adaptative lasso =========================================================
  alasso <- glmnet(x = train.sparse, 
                   y = y_train,
                   penalty.factor = 1 / abs(best.ridge))
  
  cv.alasso <- cv.glmnet(x = train.sparse, 
                         y = y_train,
                         penalty.factor = 1 / abs(best.ridge))
  
  lambda <- cv.alasso$lambda.min
  
  # define coefficients of the matrix
  coefs <- as.matrix(coef(cv.alasso))
  ix <- which(abs(coefs[, 1]) > 0)
  
  predict.alasso <- predict(alasso, test.sparse, type = 'response', s = lambda)
  ...
  cat("=============================================\n")
  cat(paste0("Model Adaptative Lasso ", i, " - Dependent Variable: ", variable), '\n')
  ...
  cat("Model Accuracy\n")
  cat("=============================================\n")
  results <- data.frame('MAE' = mae(predict.alasso, data.test[, i]),
                        'MSE' = mse(predict.alasso, data.test[, i]),
                        'RMSE' = rmse(predict.alasso, data.test[, i]),
                        'R2' = R2(predict.alasso, data.test[, i])[1])
